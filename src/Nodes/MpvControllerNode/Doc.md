...
通过本机 **mpv media-api**（`portable_config/scripts/media-api.lua`）控制播放。固定轮询保活、按排序号切换、双击播放；播放与停止均可走端口。
...
5. 从 DONE / CONNECTED 接后续逻辑节点（播放与停止都会更新 DONE）。
...
| 端口 | PLAY / INDEX / STOP → DONE / CONNECTED | TRIGGER / INDEX → DONE / CONNECTED | 同 Mpv |
