...
/**
 * Container：外壳端口按子图全部 Variable/Image/Audio 的 In/Out 汇总暴露。
 * 多个 In：各自 Out 端口依次映射到 Container In；
 * 多个 Out：各自 In 端口依次映射到 Container Out。
 */
class ContainerDataModel : public AbstractDelegateModel
{
    Q_OBJECT
public:
    ContainerDataModel();
    ...
    CustomDataFlowGraphModel &ensureInnerModel();
    CustomDataFlowGraphModel *innerModel() const { return _innerModel.get(); }
    ...
    /**
     * 将内嵌子图导出为 .childflow 文件（弹出另存为对话框）
     * @return 是否成功写入文件
     */
    bool exportChildFlow(QWidget *parent = nullptr) const;
...
private:
    ...
    /// 将当前 Out 映射值刷到外壳并对外发出 dataUpdated（load/sync 后需要）
    void flushExportOutputs();
    ...
    static bool isInterfaceIn(NodeDelegateModel const *model);
    static bool isInterfaceOut(NodeDelegateModel const *model);

    static std::shared_ptr<NodeDelegateModelRegistry> s_registry;

    std::unique_ptr<CustomDataFlowGraphModel> _innerModel;
    bool _innerSignalsConnected = false;
    bool _isSyncing = false;
    /// load/inner 整图重建期间禁止 sync，避免外壳端口反复增减拆掉外层连线
    bool _suspendInterfaceSync = false;
