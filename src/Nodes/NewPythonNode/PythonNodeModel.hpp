...
public slots:
    void doWork() {
        try {
            // 初始化Python解释器
//            qDebug()<<"run";
            std::string relative_python_path = "Python312";
            // 获取绝对路径
            std::filesystem::path absolute_python_path = std::filesystem::absolute(relative_python_path);
            // 设置环境变量
            std::string python_home_env = "PYTHONHOME=" + absolute_python_path.string();
            putenv(const_cast<char*>(python_home_env.c_str()));
            py::scoped_interpreter guard{}; // 这将自动初始化和结束Python解释器
            // 调用Python代码
...
public:
    void initPythonInterpreter()
    {
        ...
        putenv(const_cast<char*>(python_home_env.c_str()));
        // 初始化Python解释器
        py::initialize_interpreter();// 这将自动初始化和结束Python解释器
