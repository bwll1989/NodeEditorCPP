...
/**
 * @file SubtractImageOperateModel.hpp
 * @brief Image Subtract — 双路像素相减 clamp(A−B, 0, 1)，类似 TD Subtract TOP
 *
...
namespace Nodes
{
namespace SubtractImageOperateGpu
{
static const char kFragSubtract[] = R"(
...
/** @brief 双路像素相减 — 双输入 GPU 算子 */
class SubtractImageOperateModel final : public AbstractDelegateModel
{
    Q_OBJECT

public:
    SubtractImageOperateModel()
    {
        InPortCount = 2;
        OutPortCount = 1;
        CaptionVisible = true;
        Caption = "Image Subtract";
...
private:
    ...
    void requestProcess(qint64 targetTimestamp = -1)
    {
        ...
        GpuTextureHandle out =
            SubtractImageOperateGpu::run(inputAFrame.texture, inputBFrame.texture);
