...
namespace Nodes
{
class PoseEstimationMediaPipeInterface final : public QWidget
{
    Q_OBJECT
public:
    explicit PoseEstimationMediaPipeInterface(QWidget* parent = nullptr)
    {
        main_layout = new QGridLayout(this);
        main_layout->setContentsMargins(4, 2, 4, 4);
        main_layout->setSpacing(6);

        PersonScoreFilter = new FloatDragValueWidget(this);
        PersonScoreFilter->setRange(0, 1);
        PersonScoreFilter->setSingleStep(0.01);
        PersonScoreFilter->setDecimals(2);
        PersonScoreFilter->setValue(0.5);
        main_layout->addWidget(new QLabel("人体检测阈值:", this), 0, 0);
        main_layout->addWidget(PersonScoreFilter, 0, 1);
        ...
        ConfidenceFilter->setValue(0.8);
        main_layout->addWidget(new QLabel("姿态置信度:", this), 2, 0);
        ...
        DrawOverlayCheck = new QCheckBox("绘制骨架 (关闭可降 CPU)", this);
    ...
    FloatDragValueWidget* PersonScoreFilter = nullptr;
