...
#include "PoseDetectionInterface.hpp"
...
namespace Nodes
{
    class PoseDetectionDataModel : public AbstractDelegateModel
    {
        Q_OBJECT
        ...
        PoseDetectionDataModel()
        {
            ...
            connect(widget->ConfidenceFilter, &FloatDragValueWidget::valueChanged,
                    this, &PoseDetectionDataModel::setConfidence);
            connect(this, &PoseDetectionDataModel::confidenceChanged, this, [this](double value) {
            ...
            connect(widget->NmsFilter, &FloatDragValueWidget::valueChanged,
                    this, &PoseDetectionDataModel::setNmsThreshold);
            connect(this, &PoseDetectionDataModel::nmsThresholdChanged, this, [this](double value) {
            ...
            connect(widget->EnableBtn, &QPushButton::clicked,
                    this, &PoseDetectionDataModel::setEnabled);
            connect(widget->MaxFpsFilter, &FloatDragValueWidget::valueChanged,
                    this, &PoseDetectionDataModel::setMaxFps);
            connect(widget->DrawOverlayCheck, &QCheckBox::toggled,
                    this, &PoseDetectionDataModel::setDrawOverlay);
        }

        ~PoseDetectionDataModel() override
        {
    ...
    private Q_SLOTS:
            ...
            bool drawOverlay)
        {
            ...
            for (const int idx : indices) {
                ...
                const float normW = std::max(1.0f, imgWidth);
                const float normH = std::max(1.0f, imgHeight);

                cv::Point2f bodyOrigin(
                    box.x + box.width * 0.5f,
                    box.y + box.height * 0.5f);
                if (keypoints[11].x >= 0.f && keypoints[11].y >= 0.f
                    && keypoints[12].x >= 0.f && keypoints[12].y >= 0.f
                    && keypointConfs[11] > keypointThreshold
                    && keypointConfs[12] > keypointThreshold) {
                    bodyOrigin = (keypoints[11] + keypoints[12]) * 0.5f;
                }
                const float bodyScale = std::max(
                    1.0f,
                    static_cast<float>(std::max(box.width, box.height)));

                QVariantList kpList;
                QVariantList worldLandmarks;
                for (int k = 0; k < numKeypoints; ++k) {
                    const bool valid = keypointConfs[k] > keypointThreshold
                        && keypoints[k].x >= 0.f && keypoints[k].y >= 0.f;

                    QVariantMap kp;
                    if (valid) {
                        kp["x"] = static_cast<double>(keypoints[k].x);
                        kp["y"] = static_cast<double>(keypoints[k].y);
                        kp["conf"] = static_cast<double>(keypointConfs[k]);
                        kp["nx"] = static_cast<double>(keypoints[k].x / normW);
                        kp["ny"] = static_cast<double>(keypoints[k].y / normH);
                    } else {
                        kp["x"] = -1.0;
                        kp["y"] = -1.0;
                        kp["conf"] = 0.0;
                    ...
                    QVariantMap wp;
                    if (valid) {
                        wp["x"] = static_cast<double>((keypoints[k].x - bodyOrigin.x) / bodyScale);
                        wp["y"] = static_cast<double>((keypoints[k].y - bodyOrigin.y) / bodyScale);
                        wp["z"] = 0.0;
                    } else {
                        wp["x"] = 0.0;
                        wp["y"] = 0.0;
                ...
                det["box"] = boxList;
                det["bbox"] = bbox;
                det["keypoints"] = kpList;
                det["world_landmarks"] = worldLandmarks;
                detections << det;
            }

            outDetectionResults["count"] = detections.size();
            outDetectionResults["detections"] = detections;
            outDetectionResults["width"] = originalImage.cols;
            outDetectionResults["height"] = originalImage.rows;
            ...
            for (const int idx : indices) {
                ...
                const std::string confText =
                    "Person " + std::to_string(static_cast<int>(scores[idx] * 100)) + "%";
                ...
                for (const auto& bone : skeleton) {
                    ...
                    if (pt1.x > 0 && pt1.y > 0 && pt2.x > 0 && pt2.y > 0 &&
                        keypointConfs[kp1] > keypointThreshold &&
                        keypointConfs[kp2] > keypointThreshold) {
        ...
        bool initializeOnnxSession()
        {
            ...
            if (!m_ortEnv) {
                m_ortEnv = std::make_unique<Ort::Env>(OrtLoggingLevel::ORT_LOGGING_LEVEL_WARNING, "PoseONNX");
            }
    ...
    private:
        ...
        PoseDetectionInterface* widget = new PoseDetectionInterface();
