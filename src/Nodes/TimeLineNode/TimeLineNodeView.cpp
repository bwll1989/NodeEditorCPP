...
TimeLineNodeView::TimeLineNodeView(TimeLineNodeModel* model, QWidget *parent) : BaseTimelineView(model, parent)
{

    //将时间游标的显示与模型中的时钟相绑定
    auto* derivedModel = dynamic_cast<TimeLineNodeModel*>(getModel());
    // 连接时间码生成器信号
...
void TimeLineNodeView::movePlayheadToFrame(int frame)
{
...
void TimeLineNodeView::mouseMoveEvent(QMouseEvent *event)
{
    // 处理右键事件
...
void TimeLineNodeView::initToolBar(BaseTimelineToolbar *toolbar)
{
    ...
    // 连接工具栏停止按钮信号
    connect(dynamic_cast<TimeLineNodeToolBar*>(m_toolbar), &TimeLineNodeToolBar::stopClicked, [this]() {
        getModel()->onStopPlay();
    });
    // 连接工具栏暂停按钮信号
    connect(dynamic_cast<TimeLineNodeToolBar*>(m_toolbar), &TimeLineNodeToolBar::pauseClicked, [this]() {
        getModel()->onPausePlay();
    });
    connect(dynamic_cast<TimeLineNodeToolBar*>(m_toolbar),&TimeLineNodeToolBar::prevFrameClicked,[this](){
       getModel()->onSetPlayheadPos(qMax(0,getModel()->getPlayheadPos()-1));
    });
    connect(dynamic_cast<TimeLineNodeToolBar*>(m_toolbar),&TimeLineNodeToolBar::nextFrameClicked,[this](){
        getModel()->onSetPlayheadPos(getModel()->getPlayheadPos()+1);
    });
    // 连接移动剪辑按钮信号
    connect(dynamic_cast<TimeLineNodeToolBar*>(m_toolbar), &TimeLineNodeToolBar::moveClipClicked, [this](int dx) {
        moveSelectedClip(dx,0,false);
    });
    // 连接删除剪辑按钮信号
    connect(dynamic_cast<TimeLineNodeToolBar*>(m_toolbar), &TimeLineNodeToolBar::deleteClipClicked, [this]() {
    ...
    // 连接放大按钮信号
    connect(dynamic_cast<TimeLineNodeToolBar*>(m_toolbar), &TimeLineNodeToolBar::zoomInClicked, [this]() {
    ...
    // 连接缩小按钮信号
    connect(dynamic_cast<TimeLineNodeToolBar*>(m_toolbar), &TimeLineNodeToolBar::zoomOutClicked, [this]() {
    ...
    connect( dynamic_cast<TimeLineNodeModel*>(getModel())->getClock(), &TimeLineNodeClock::timecodePlayingChanged,
       dynamic_cast<TimeLineNodeToolBar*>(this->m_toolbar),&TimeLineNodeToolBar::setPlaybackState);
    // 连接循环按钮信号
    connect(dynamic_cast<TimeLineNodeToolBar*>(m_toolbar), &TimeLineNodeToolBar::loopToggled, [this](bool loop) {
        dynamic_cast<TimeLineNodeModel*>(getModel())->onSetLoop(loop);
    });

    connect( dynamic_cast<TimeLineNodeModel*>(getModel())->getClock(), &TimeLineNodeClock::loopingChanged,
       dynamic_cast<TimeLineNodeToolBar*>(this->m_toolbar),&TimeLineNodeToolBar::setLoopState);
