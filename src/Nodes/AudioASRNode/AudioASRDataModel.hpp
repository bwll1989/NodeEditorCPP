...
namespace Nodes {
     ...
     */
    // 修改后的AudioASRDataModel类（关键部分）
    class AudioASRDataModel : public AbstractDelegateModel
    {
        Q_OBJECT
    ...
    public:
         ...
         */
        AudioASRDataModel()
            : _worker(new AudioASRWorker())
            , _workerThread(new QThread(this))
        {
            InPortCount = 8;
            OutPortCount = 8;
            widget = new AudioASRInterface(InPortCount, OutPortCount);
            CaptionVisible = true;
            ...
            connect(_workerThread, &QThread::finished, _worker, &AudioASRWorker::stopProcessing);
            connect(widget->mMatrixWidget, &MatrixWidget::valueChanged, this, &AudioASRDataModel::setMatrix);
            connect(_worker, &AudioASRWorker::processingStatusChanged, this, &AudioASRDataModel::onProcessingStatusChanged);
