...
namespace Nodes
{
     ...
     * @param parent 父对象
     */
    AudioASRWorker::AudioASRWorker(QObject *parent)
        : QObject(parent)
        , _isProcessing(false)
     ...
     * @brief 开始处理音频数据
     */
    void AudioASRWorker::startProcessing()
    {
        QMutexLocker locker(&_mutex);
        if (_isProcessing) {
            qDebug() << "AudioASRWorker: Already processing, ignoring duplicate call";
            return;
        }
        _isProcessing = true;
        _lastProcessedTimestamp = 0;
     ...
     * @brief 停止处理音频数据
     */
    void AudioASRWorker::stopProcessing()
    {
        QMutexLocker locker(&_mutex);
     ...
     * @param matrix 新的矩阵数据
     */
    void AudioASRWorker::updateMatrix(const Eigen::MatrixXd& matrix)
    {
        QMutexLocker locker(&_mutex);
        _matrix = matrix;
    }
     ...
     * @brief 处理音频数据的主循环
     */
    void AudioASRWorker::processAudioData()
    {
     ...
     */
    void AudioASRWorker::onFrameTick(qint64 frameCount)
    {
        if (!_isProcessing || _inputBuffers.empty() || _outputBuffers.empty()) {
            return;
        }
     ...
     * @param timestamp 时间戳
     */
    void AudioASRWorker::performMatrixOperation(const std::vector<AudioFrame>& inputFrames, qint64 timestamp)
    {
 ...
 * @param matrix 矩阵数据
 */
void AudioASRWorker::initializeBuffers(int inputCount, int outputCount, const Eigen::MatrixXd& matrix)
{
    // 停止当前处理
    if (_isProcessing) {
        stopProcessing();
 ...
 * @param buffer 音频缓冲区
 */
void AudioASRWorker::setInputBuffer(int port, std::shared_ptr<AudioTimestampRingQueue> buffer)
{
    QMutexLocker locker(&_mutex);
    ...
    if (port >= 0 && port < static_cast<int>(_inputBuffers.size())) {
        ...
        qWarning() << "AudioASRWorker: Invalid input port index:" << port 
 ...
 * @return 输出音频缓冲区
 */
std::shared_ptr<AudioTimestampRingQueue> AudioASRWorker::getOutputBuffer(int port)
{
