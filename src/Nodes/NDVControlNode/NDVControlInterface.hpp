...
namespace Nodes
{
    ...
    public:
        explicit NDVControlInterface(QWidget *parent = nullptr){
            // IP 和端口设置在第一行
            main_layout->addWidget(IP, 0, 0, 1, 1);        // IP 地址占两列
            main_layout->addWidget(Port, 0, 1, 1, 1);      // 端口号占一列
            ...
            main_layout->addWidget(Play, 2, 0, 1, 2);           // 播放按钮
            main_layout->addWidget(LoopPlay, 3, 0, 1, 2);      // 循环播放按钮

            main_layout->addWidget(Stop, 4, 0, 1, 2);
            // 关闭所有客户端在第五行
            main_layout->addWidget(ClosePC, 5, 0, 1, 2);   // 关闭所有客户端，占三列
