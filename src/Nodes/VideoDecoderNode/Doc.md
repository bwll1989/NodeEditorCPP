...
## 技术实现
- **解码引擎**：基于 FFmpeg (VideoDecoder 类) 实现，支持音视频同步解码
- **数据传输**：
  - 音频：使用 `AudioTimestampRingQueue` 环形缓冲区进行跨线程数据传输
  - 视频：通过 `NodeDataTypes::ImageData` (OpenCV Mat) 传输 RGB 帧
- **音频处理**：
  - **重采样**：自动将不同采样率/格式的音频转换为 48000Hz Float 格式
  - **音量控制**：支持分贝(dB)级音量调节，包含线性增益转换和防削波(Clipping)保护
  - **固定切片**：输出固定 1920 采样/声道的音频帧，确保低延迟和播放稳定性
- **线程模型**：独立 QThread 线程进行解码和处理，主线程负责 UI 更新
- **内存管理**：使用 `std::shared_ptr` 管理数据生命周期，FFmpeg 缓冲区使用 `av_mallocz` 对齐分配
