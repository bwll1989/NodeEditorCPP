...
(function(){
  ...
  window.EPWidgets.createEPVFloatSliderWidget = function(grid, initialProps = {}, opts = {}) {
    ...
    const defaultTitle = initialProps.title || '竖向浮点滑块';
    const { node, mountNode } = createContainer(grid, defaultTitle, 4, 10, opts);
    
    // 1. 同步注册基础 API
    registerNode(node, '竖向浮点滑块', {
      getProps(){ return { 
    ...
    loadTemplate('widgets/VFloatSlider/widget.html').then(template => {
      const app = vue.createApp({
        ...
        mounted() {
          // 函数级注释：初始化竖向浮点柱状条渲染与交互（统一 Pointer 事件，优化移动端）
      ...
      registerNode(node, '竖向浮点滑块', {
        ...
        setProps(p){ 
          ...
          if (p.step!==undefined) vm.step = Number(p.step);
