...
export const widgetMeta: WidgetMeta = {
  type: '3D折线',
  factory: 'createEPLine3DWidget',
  defaultW: 20,
  defaultH: 14,
  defaults: {
    commandId: '/cmd/line3d',
    bgColor: '#0f172a',
    fontSize: '14',
    points: [],
    incoming: null,
...
echarts.use([TooltipComponent, CanvasRenderer, Line3DChart, Grid3DComponent]);
    ...
    series: [
      {
        ...
        data,
        lineStyle: {
          width: Number(s.lineWidth) || 3,
          color: s.lineColor || '#38bdf8',
          opacity: 0.95,
        },
      },
    ],
...
watch(
  () => [s.points, s.lineWidth, s.lineColor, s.xLabel, s.yLabel, s.zLabel, s.showAxes, s.autoRotate, s.bgColor] as const,
  () => renderChart(false),
  { deep: true },
);
</script>
...
<template>
  <div ref="rootRef" class="ep-line3d-root" :style="shellStyle">
    <div ref="chartRef" class="ep-line3d-chart" />
    <div class="ep-line3d-views" @pointerdown.stop @mousedown.stop>
      <button
        type="button"
        class="ep-line3d-view-btn ep-line3d-view-cycle"
    ...
    <div class="ep-line3d-meta">{{ (s.points && s.points.length) || 0 }} pts</div>
  </div>
</template>
...
.ep-line3d-view-btn:hover {
  border-color: rgba(56, 189, 248, 0.7);
  color: #fff;
}
.ep-line3d-view-btn.active {
  background: rgba(56, 189, 248, 0.28);
  border-color: rgba(56, 189, 248, 0.85);
  color: #fff;
}
.ep-line3d-view-cycle {
  font-weight: 600;
}
.ep-line3d-meta {
  position: absolute;
