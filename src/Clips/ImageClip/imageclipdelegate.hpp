...
class ImageClipDelegate : public AbstractClipDelegate {
    Q_OBJECT
public:
    explicit ImageClipDelegate(QObject *parent = nullptr) 
    ...
    QWidget *createEditor(QWidget *parent, const QStyleOptionViewItem &option, const QModelIndex &index) const override {
        ...
        auto* self = const_cast<ImageClipDelegate*>(this);
        self->m_model = qobject_cast<ImageClipModel*>(clip);
        if (!self->m_model) {
            qDebug() << "Failed to cast to ImageClipModel";
            return editor;
        }
...
private:
    ImageClipModel* m_model;  // 使用 m_ 前缀表示成员变量
    QSpinBox* m_xSpinBox;
    QSpinBox* m_ySpinBox;
