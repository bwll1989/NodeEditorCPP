...
import type { LovelaceBarEntityConfig } from "../../../types";
import "../../../components/ha-entity-picker";
import "../../../components/ha-color-picker";

@customElement("flow-bar-entities-editor")
export class FlowBarEntitiesEditor extends LitElement {
  @property({ attribute: false }) public entities: LovelaceBarEntityConfig[] = [];

  private _emit(entities: LovelaceBarEntityConfig[]): void {
    this.dispatchEvent(
  ...
  private _add(): void {
    this._emit([...this.entities, { entity: "", name: "" }]);
  }
  ...
  private _patch(index: number, patch: Partial<LovelaceBarEntityConfig>): void {
    ...
    return html`
      <div class="editor">
        ...
        ${this.entities.length === 0
          ? html`<p class="empty">添加动作后，每根柱显示对应动作的实时数值</p>`
          : this.entities.map(
              (item, index) => html`
                <div class="row">
...
declare global {
  interface HTMLElementTagNameMap {
    "flow-bar-entities-editor": FlowBarEntitiesEditor;
  }
}
